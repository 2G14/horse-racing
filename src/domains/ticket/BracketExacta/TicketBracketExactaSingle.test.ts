...
Deno.test('create: 枠番を降順で与えた時、そのまま生成される', () => {
...
Deno.test('equals: 枠番1,2が同じ時、馬券は同じ馬券の判定になる', () => {
  ...
  const ticket1 = unsafe(TicketBracketExactaSingle.create(
    bracketNumber1,
    bracketNumber2,
  ));
  const ticket2 = unsafe(TicketBracketExactaSingle.create(
    bracketNumber1,
    bracketNumber2,
  ));
