import TicketExactaSingle from '@/domains/ticket/Exacta/TicketExactaSingle.ts';
...
Deno.test('create: 馬番を昇順で与えた時、そのまま生成される', () => {
  ...
  const ticketResult = TicketExactaSingle.create(
    horseNumber1,
    horseNumber2,
  );
...
Deno.test('create: 馬番を降順で与えた時、そのまま生成される', () => {
...
Deno.test('equals: 馬番1,2が同じ時、馬券は同じ馬券の判定になる', () => {
  ...
  const ticket1 = unsafe(TicketExactaSingle.create(
    horseNumber1,
    horseNumber2,
  ));
  const ticket2 = unsafe(TicketExactaSingle.create(
    horseNumber1,
    horseNumber2,
  ));
