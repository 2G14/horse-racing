import TicketWinSingle from '@/domains/ticket/Win/TicketWinSingle.ts';
...
Deno.test('create: 馬番を与えた時、生成される', () => {
  ...
  const ticketResult = TicketWinSingle.create(horseNumber);
...
Deno.test('equals: 馬番が同じ時、馬券は同じ馬券の判定になる', () => {
  ...
  const ticket1 = unsafe(TicketWinSingle.create(horseNumber));
  const ticket2 = unsafe(TicketWinSingle.create(horseNumber));
...
Deno.test('equals: 馬番が異なる時、馬券は異なる馬券の判定になる', () => {
  ...
  const ticket1 = unsafe(TicketWinSingle.create(horseNumber1));
  const ticket2 = unsafe(TicketWinSingle.create(horseNumber2));
